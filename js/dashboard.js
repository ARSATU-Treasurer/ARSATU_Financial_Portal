document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (logoutBtn) {
        logoutBtn.addEventListener('click', async () => {
            ...
            localStorage.removeItem('loginSource'); window.location.replace('index.html');
        });
    }

    async function getProfileMap() {
        const { data } = await supabaseClient.from('profiles').select('id, full_name, department');
        const map = {}; if (data) data.forEach(p => map[p.id] = p); return map;
    }
    ...
    window.loadPendingDonations = async function() {
        const tbody = document.querySelector('#pending-donations-table tbody'); if (!tbody) return;
        try {
            ...
            const pMap = await getProfileMap(); const { data: banks } = await supabaseClient.from('bank_accounts').select('*'); const bankOpts = banks ? banks.map(b => `<option value="${b.id}">${b.bank_name}</option>`).join('') : ''; const { data: funds } = await supabaseClient.from('funds').select('*'); const fundOpts = funds ? funds.map(f => `<option value="${f.id}">${f.fund_name}</option>`).join('') : '';

            tbody.innerHTML = data.map(req => {
                const date = req.transaction_date ? new Date(req.transaction_date).toLocaleDateString('th-TH') : '-';
                let typeLabel = '';
                if(req.transaction_type === 'donation_cash') typeLabel = '💝 บริจาค (เงินสด)'; else if(req.transaction_type === 'donation_transfer') typeLabel = '💝 บริจาค (โอน)'; else if(req.transaction_type === 'income') typeLabel = '📈 รายรับเข้าชุมนุม'; else typeLabel = '📉 รายจ่าย (หักออก)';
                ...
                return `<tr><td>${date}</td><td style="color:${color}; font-weight:500;">${typeLabel}</td><td>${req.description || '-'}${locationText}<br><small style="color:gray;">ผู้แจ้ง: ${uName}</small></td><td style="font-weight:bold; color:${color};">฿${parseFloat(req.amount).toLocaleString()}</td><td>${slipLink}</td><td><div style="display:flex; flex-direction:column; gap:5px;"><select id="bank-for-${req.id}" style="padding:5px; border-radius:4px; font-size:12px;"><option value="">-- เลือกบัญชี --</option>${bankOpts}</select><select id="fund-for-${req.id}" style="padding:5px; border-radius:4px; font-size:12px;"><option value="">-- เลือกกองทุน --</option>${fundOpts}</select><button onclick="approveDonation('${req.id}', ${req.amount}, '${req.transaction_type}')" class="btn btn-success" style="padding:5px 10px; font-size:12px;">✅ อนุมัติ</button></div></td></tr>`;
            }).join('');
        } catch (e) {}
    };

    window.approveDonation = async function(id, amount, type) {
        const bankId = document.getElementById(`bank-for-${id}`)?.value; const fundId = document.getElementById(`fund-for-${id}`)?.value;
        ...
        if (confirm(`ยืนยันนำรายการ ฿${amount.toLocaleString()} เข้าบัญชีและกองทุน?`)) {
            try {
                const { data: bData } = await supabaseClient.from('bank_accounts').select('balance').eq('id', bankId).single(); let nBal = parseFloat(bData.balance); nBal += (type === 'expense' ? -parseFloat(amount) : parseFloat(amount)); await supabaseClient.from('bank_accounts').update({ balance: nBal }).eq('id', bankId);
                const { data: fData } = await supabaseClient.from('funds').select('remaining_budget').eq('id', fundId).single(); let nFun = parseFloat(fData.remaining_budget); nFun += (type === 'expense' ? -parseFloat(amount) : parseFloat(amount)); await supabaseClient.from('funds').update({ remaining_budget: nFun }).eq('id', fundId);
    ...
    window.loadPendingRequests = async function() {
        const tbody = document.querySelector('#requests-table tbody'); if (!tbody) return;
        try {
            ...
            tbody.innerHTML = data.map(req => {
                const date = new Date(req.created_at).toLocaleDateString('th-TH'); const amt = req.status === 'pending_advance' ? req.requested_amount : req.total_actual_amount;
                let typeL = '', statL = '', btnL = '';
                
                if (req.status === 'pending_advance') { 
                    typeL = '<span style="color:var(--info); font-weight:bold;">ขอเบิกล่วงหน้า</span>'; statL = '<span class="status-badge" style="background:#fef3c7; color:#d97706;">รอโอนตั้งต้น</span>'; btnL = `<button onclick="openModal('${req.id}')" class="btn btn-warning" style="padding:6px 12px; font-size:12px; margin-bottom:5px;">💸 โอนเงิน</button>`;
                } else { 
                    typeL = req.request_type === 'advance' ? '<span style="color:var(--info); font-weight:bold;">เคลียร์บิล (ล่วงหน้า)</span>' : '<span style="color:var(--primary); font-weight:bold;">สำรองจ่าย (เบิกคืน)</span>'; statL = '<span class="status-badge" style="background:#fee2e2; color:#ef4444;">รอตรวจบิล</span>'; btnL = `<button onclick="openModal('${req.id}')" class="btn btn-info" style="padding:6px 12px; font-size:12px; margin-bottom:5px;">🔍 ตรวจบิล</button>`;
                }
                ...
                return `<tr><td>${date}</td><td>${uName} ${deptBadge}</td><td>${typeL}</td><td>${req.purpose}</td><td style="font-weight:bold;">฿${amt.toLocaleString()}</td><td>${statL}</td><td><div style="display:flex; flex-direction:column;">${btnL}${editBtn}${rejectBtn}</div></td></tr>`;
            }).join('');
        } catch (e) {}
    };
    ...
    window.rejectRequest = async function(id) {
        ...
        if (reason === null) return; if (reason.trim() === '') return alert('กรุณาระบุเหตุผลด้วยครับ');
        try {
            const { data: c } = await supabaseClient.from('clearances').select('request_type, status').eq('id', id).single();
            let newStatus = 'cancelled', actionType = 'reject_clearance', alertText = '✅ ยกเลิกรายการสำเร็จ';
            if (c.request_type === 'advance' && c.status === 'pending_clearance') { newStatus = 'advance_transferred'; actionType = 'return_clearance'; alertText = '✅ ตีกลับบิลให้ Member แก้ไขสำเร็จ'; }
            await supabaseClient.from('clearances').update({ status: newStatus }).eq('id', id);
            await supabaseClient.from('audit_logs').insert([{ clearance_id: id, admin_id: currentUser.id, action_type: actionType, old_value: `สถานะเดิม: ${c.status}`, new_value: newStatus === 'cancelled' ? 'ยกเลิกบิลทิ้ง' : 'ตีกลับบิลให้แก้ไขใหม่', edit_reason: reason }]);
            alert(alertText); window.loadPendingRequests(); window.loadClearanceHistory(); if (typeof window.loadAuditLogs === 'function') window.loadAuditLogs();
        } catch (err) { alert('❌ เกิดข้อผิดพลาด: ' + err.message); }
    };

    window.openEditModal = async function(id) {
        document.getElementById('edit-req-modal').style.display = 'flex'; document.getElementById('edit-msg').textContent = 'กำลังโหลดข้อมูล...'; document.getElementById('edit-items-tbody').innerHTML = '';
        try {
            ...
            document.getElementById('edit-req-id').value = c.id; document.getElementById('edit-req-type').value = c.request_type;
            const deptSelect = document.getElementById('edit-req-dept'); if (c.department && c.department !== '-') deptSelect.value = c.department;
            document.getElementById('edit-req-purpose').value = c.purpose; document.getElementById('edit-req-reason').value = ''; 
            
            const advSec = document.getElementById('edit-advance-section'); const amtInput = document.getElementById('edit-req-amount');
            if (c.request_type === 'advance' && c.status === 'pending_advance') { advSec.style.display = 'block'; amtInput.value = c.requested_amount; document.getElementById('edit-items-section').style.display = 'none'; } 
            else {
                ...
                if (items && items.length > 0) {
                    items.forEach(it => {
                        ...
                        tr.innerHTML = `<td><input type="hidden" class="edit-item-id" value="${it.id}"><input type="text" class="edit-item-name" value="${it.item_name.replace(/"/g, '&quot;')}" style="width:100%; padding:5px;" required></td><td><input type="number" class="edit-item-qty" min="1" value="${it.quantity}" style="width:100%; padding:5px; text-align:center;" required></td><td><input type="number" class="edit-item-price" min="0" step="0.01" value="${it.total_price}" style="width:100%; padding:5px; text-align:right;" required></td><td style="text-align: center;"><button type="button" class="btn btn-danger edit-del-btn" style="padding: 4px 8px; font-size: 11px;">ลบ</button></td>`;
                        tbody.appendChild(tr); tr.querySelectorAll('input').forEach(i => i.addEventListener('input', window.calculateEditTotal)); tr.querySelector('.edit-del-btn')?.addEventListener('click', () => { tr.remove(); window.calculateEditTotal(); });
                    });
                }
            }
    ...
    window.calculateEditTotal = function() { let total = 0; document.querySelectorAll('.edit-item-price').forEach(inp => { total += parseFloat(inp.value) || 0; }); document.getElementById('edit-total-actual').textContent = total.toLocaleString(); };
    ...
    document.getElementById('edit-add-item-btn')?.addEventListener('click', () => {
        ...
        tr.innerHTML = `<td><input type="hidden" class="edit-item-id" value="new"><input type="text" class="edit-item-name" placeholder="ชื่อรายการ" style="width:100%; padding:5px;" required></td><td><input type="number" class="edit-item-qty" min="1" value="1" style="width:100%; padding:5px; text-align:center;" required></td><td><input type="number" class="edit-item-price" min="0" step="0.01" value="0" style="width:100%; padding:5px; text-align:right;" required></td><td style="text-align: center;"><button type="button" class="btn btn-danger edit-del-btn" style="padding: 4px 8px; font-size: 11px;">ลบ</button></td>`;
        tbody.appendChild(tr); tr.querySelectorAll('input').forEach(i => i.addEventListener('input', window.calculateEditTotal)); tr.querySelector('.edit-del-btn')?.addEventListener('click', () => { tr.remove(); window.calculateEditTotal(); });
    });
    ...
    if (editForm) {
        editForm.addEventListener('submit', async (e) => {
            e.preventDefault(); const btn = document.getElementById('save-edit-btn'); const msg = document.getElementById('edit-msg'); btn.disabled = true; if (msg) { msg.style.color = 'var(--primary)'; msg.textContent = 'กำลังบันทึกข้อมูลและประวัติ...'; }

            const reqId = document.getElementById('edit-req-id').value; const reqType = document.getElementById('edit-req-type').value; const dept = document.getElementById('edit-req-dept').value; const purpose = document.getElementById('edit-req-purpose').value; const reason = document.getElementById('edit-req-reason').value; const isSubstitute = document.getElementById('edit-is-substitute')?.value === 'true';
            ...
            try {
                ...
                if (reqType === 'advance' && oldData.status === 'pending_advance' && !isSubstitute) {
                    const newAmt = parseFloat(document.getElementById('edit-req-amount').value); updatePayload.requested_amount = newAmt; logDetails += `, แก้ไขยอดเงินล่วงหน้าจาก ${oldData.requested_amount} เป็น ${newAmt}`;
                } else {
                    ...
                    await supabaseClient.from('clearance_items').delete().eq('clearance_id', reqId); if (currentItems.length > 0) await supabaseClient.from('clearance_items').insert(currentItems);
                }
                ...
                if (isSubstitute) {
                    ...
                    if (!sFile) throw new Error("กรุณาแนบใบเสร็จ / Statement ด้วยครับ"); if (msg) msg.textContent = 'กำลังอัปโหลดไฟล์หลักฐาน...'; let sUrl = null, rUrl = null;
    ...
    window.openSubstituteClearance = async function(id) {
        ...
        document.getElementById('edit-reason-container').style.display = 'none'; const btn = document.getElementById('save-edit-btn'); if (btn) { btn.innerHTML = '🚀 ส่งบิลเคลียร์เงิน (แทน Member)'; btn.className = 'btn btn-primary'; }
    };

    const actionModal = document.getElementById('action-modal');
    window.openModal = async function(id) {
        if(actionModal) actionModal.style.display = 'flex'; const msg = document.getElementById('modal-msg'); if(msg) { msg.style.color = 'var(--info)'; msg.textContent = 'กำลังดึงข้อมูล...'; }
    ...
    window.closeModal = function() { if(actionModal) actionModal.style.display = 'none'; document.getElementById('admin-action-form')?.reset(); };

    window.recalculateAdminTotal = function() {
        const c = window.currentClearance; if(!c) return; let totalAppr = 0, isEdited = false;
        ...
        const amtP = document.getElementById('modal-amount-display'); const title = document.getElementById('modal-title'); const slipSec = document.getElementById('admin-slip-section');
        const bankSel = document.getElementById('admin-bank-select'); const fundSel = document.getElementById('admin-fund-select'); const bankFundWrapper = document.getElementById('admin-bank-fund-wrapper');
        const bankDiv = document.getElementById('modal-member-bank'); const bankText = document.getElementById('modal-bank-text');

        if (bankSel) bankSel.required = false; if (fundSel) fundSel.required = false;

        if(bankDiv && bankText) { if(actionDir === 'pay' && c.member_bank_details) { bankDiv.style.display = 'block'; bankText.textContent = c.member_bank_details; } else { bankDiv.style.display = 'none'; } }
        ...
        if (actionDir === 'pay') { 
            ...
            if(slipSec) slipSec.style.display = 'block'; if(bankFundWrapper) bankFundWrapper.style.display = 'flex'; if (bankSel) bankSel.required = true; if (fundSel) fundSel.required = true;
        } 
            ...
            if(slipSec) slipSec.style.display = 'none'; if(bankFundWrapper) bankFundWrapper.style.display = 'flex'; if (bankSel) bankSel.required = true; if (fundSel) fundSel.required = true;
        } 
        else { 
            ...
            if(slipSec) slipSec.style.display = 'none'; if(bankFundWrapper) bankFundWrapper.style.display = 'none'; 
        }
    ...
    if (modalForm) {
        modalForm.addEventListener('submit', async (e) => {
            e.preventDefault(); const msg = document.getElementById('modal-msg'); const btn = document.getElementById('confirm-action-btn');
            if(btn) btn.disabled = true; if(msg) { msg.style.color = 'var(--info)'; msg.textContent = 'กำลังประมวลผล...'; }

            const reqId = document.getElementById('modal-req-id')?.value; const finalTotal = parseFloat(document.getElementById('modal-final-total')?.value); const processAmt = parseFloat(document.getElementById('modal-req-amount')?.value); const actionDir = document.getElementById('modal-action-dir')?.value; const bankId = document.getElementById('admin-bank-select')?.value; const fundId = document.getElementById('admin-fund-select')?.value; const slipFile = document.getElementById('admin-slip')?.files[0];
            ...
            try {
                const inputs = document.querySelectorAll('.admin-edit-price');
                if (inputs.length > 0) { for (let inp of inputs) { const iId = inp.dataset.id; const val = parseFloat(inp.value)||0; const orig = parseFloat(inp.dataset.original)||0; if (val !== orig) await supabaseClient.from('clearance_items').update({ total_price: val }).eq('id', iId); } }
                ...
                if (actionDir !== 'none' && processAmt > 0) {
                    const { data: bData } = await supabaseClient.from('bank_accounts').select('balance').eq('id', bankId).single(); const { data: fData } = await supabaseClient.from('funds').select('remaining_budget').eq('id', fundId).single();
                    ...
                    const reqPurpose = window.currentClearance.purpose || '-'; const shortId = reqId.substring(0,6); const logDesc = window.currentClearance.status === 'pending_advance' ? `[โอนตั้งต้น] ${reqPurpose} (${shortId})` : `[เคลียร์บิล] ${reqPurpose} (${shortId})`;
                ...
                const upData = { status: newStat, total_actual_amount: finalTotal }; if (aSlipUrl) upData.admin_transfer_slip = aSlipUrl;
                ...
                const reqPurpose = window.currentClearance?.purpose || '-'; const actionText = newStat === 'advance_transferred' ? '💸 แอดมินโอนเงินล่วงหน้าให้แล้ว' : '✅ แอดมินอนุมัติเคลียร์บิลสำเร็จ'; const alertMsg = `${actionText}\n\n📌 หัวข้อ: ${reqPurpose}\n💰 ยอดอนุมัติ: ฿${finalTotal.toLocaleString()}\n\n(บันทึกลงสมุดบัญชีเรียบร้อย)`;
                if (window.sendLineMessage) window.sendLineMessage(alertMsg);
    ...
    window.loadLedger = async function() {
        ...
        const tbody = document.querySelector('#ledger-table tbody'); if (!tbody) return;
        try {
            ...
            const pMap = await getProfileMap(); const { data: bData } = await supabaseClient.from('bank_accounts').select('id, bank_name'); const { data: fData } = await supabaseClient.from('funds').select('id, fund_name');
            const bankMap = {}, fundMap = {}; if(bData) bData.forEach(b => bankMap[b.id] = b.bank_name); if(fData) fData.forEach(f => fundMap[f.id] = f.fund_name);

            tbody.innerHTML = data.map(tx => {
                const date = tx.transaction_date ? new Date(tx.transaction_date).toLocaleDateString('th-TH') : new Date(tx.created_at).toLocaleDateString('th-TH'); const amt = parseFloat(tx.amount) || 0;
                let inc = '-', exp = '-'; if (tx.transaction_type === 'expense') { exp = `<span style="color:var(--danger); font-weight:bold;">฿${amt.toLocaleString()}</span>`; } else { inc = `<span style="color:var(--success); font-weight:bold;">฿${amt.toLocaleString()}</span>`; }
                ...
                return `<tr><td>${date}</td><td style="text-align:center;">${deptLabel}</td><td>${tx.description || '-'}${tx.location ? ` (ส: ${tx.location})` : ''}</td><td style="font-size:12px; color:gray;">🏦 ${bankMap[tx.bank_account_id]||'-'}<br>💼 ${fundMap[tx.fund_id]||'-'}</td><td style="text-align:right; background:#f0fdf4;">${inc}</td><td style="text-align:right; background:#fef2f2;">${exp}</td><td style="font-size:13px; color:var(--text-muted);">${pMap[tx.created_by]?.full_name || 'ระบบ / แอดมิน'}</td><td style="text-align:center;"><button onclick="viewTransaction('${tx.id}')" class="btn btn-outline" style="padding:4px 8px; font-size:12px;">🔍 ดูสลิป</button></td></tr>`;
            }).join('');
        } catch (e) {}
    };
    ...
    if (directForm) {
        directForm.addEventListener('submit', async (e) => {
            e.preventDefault(); const msg = document.getElementById('direct-msg'); const subBtn = directForm.querySelector('button'); subBtn.disabled = true;
            if(msg) { msg.style.color = 'var(--info)'; msg.textContent = 'กำลังบันทึก...'; }
            try {
                const type = document.getElementById('direct-type').value; const desc = document.getElementById('direct-desc').value; const amt = parseFloat(document.getElementById('direct-amount').value); const bankId = document.getElementById('direct-bank').value; const fundId = document.getElementById('direct-fund').value;
                const { data: bData } = await supabaseClient.from('bank_accounts').select('balance').eq('id', bankId).single(); const { data: fData } = await supabaseClient.from('funds').select('remaining_budget').eq('id', fundId).single();
                let nBal = parseFloat(bData.balance), nFun = parseFloat(fData.remaining_budget);
                if (type === 'income') { nBal += amt; nFun += amt; } else { nBal -= amt; nFun -= amt; }
                await supabaseClient.from('bank_accounts').update({ balance: nBal }).eq('id', bankId); await supabaseClient.from('funds').update({ remaining_budget: nFun }).eq('id', fundId);
    ...
    window.loadClearanceHistory = async function() {
        const tbody = document.querySelector('#clearance-history-table tbody'); if (!tbody) return;
        try {
            const { data, error } = await supabaseClient.from('clearances').select('*').order('created_at', { ascending: false }); 
            if (error) throw error; if (!data || data.length === 0) { tbody.innerHTML = `<tr><td colspan="7" style="text-align:center; color:gray;">ไม่มีประวัติ</td></tr>`; return; }

            const pMap = await getProfileMap();

            tbody.innerHTML = data.map(req => {
                const date = new Date(req.created_at).toLocaleDateString('th-TH'); const name = pMap[req.member_id]?.full_name || 'Unknown'; const typeLabel = req.request_type === 'advance' ? 'เบิกล่วงหน้า' : 'สำรองจ่าย'; const amt = req.total_actual_amount > 0 ? req.total_actual_amount : req.requested_amount;
                let stat = '', btn = `<button type="button" onclick="viewClearance('${req.id}')" class="btn btn-info" style="padding:4px 8px; font-size:12px; width:100%;">🔍 ดูบิลย่อย</button>`;
                ...
                else if (req.status === 'advance_transferred') { stat = '<span class="status-badge" style="background:#dbeafe; color:#2563eb;">💸 รอ Member เคลียร์</span>'; btn = `<div style="display:flex; gap:5px; justify-content:center; flex-direction:column;"><button type="button" onclick="viewClearance('${req.id}')" class="btn btn-info" style="padding:4px 8px; font-size:12px;">🔍 ดู</button><button type="button" onclick="openSubstituteClearance('${req.id}')" class="btn btn-warning" style="padding:4px 8px; font-size:12px; color:white;">📝 เคลียร์แทน</button></div>`; }
                ...
                const cwBtn = `<button type="button" onclick="openCoWorkerModal('${req.id}')" class="btn btn-outline" style="padding:4px 8px; font-size:11px; display:block; margin-top:5px; width:100%; border-color:var(--primary); color:var(--primary);">👥 จัดการ CoWorker</button>`;
    ...
    window.loadAuditLogs = async function() {
        const tbody = document.querySelector('#audit-log-table tbody'); if (!tbody) return;
        try {
            const { data, error } = await supabaseClient.from('audit_logs').select('*').order('created_at', { ascending: false }).limit(100);
            if (error) throw error; if (!data || data.length === 0) { tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; color:gray;">ไม่มีประวัติการแก้ไข / ยกเลิก</td></tr>`; return; }

            const pMap = await getProfileMap(); const { data: cData } = await supabaseClient.from('clearances').select('id, purpose'); const cMap = {}; if(cData) cData.forEach(c => cMap[c.id] = c.purpose);

            tbody.innerHTML = data.map(log => {
                const date = new Date(log.created_at).toLocaleString('th-TH'); const adminName = pMap[log.admin_id]?.full_name || 'Admin'; const purpose = cMap[log.clearance_id] || 'บิลถูกลบไปแล้ว';
                let actionBadge = '';
                ...
                return `<tr><td style="font-size:12px;">${date}</td><td style="font-weight:bold;">${adminName}</td><td>${actionBadge}</td><td>${purpose}</td><td style="color:var(--danger); font-weight:500;">${log.edit_reason || '-'}</td><td style="font-size:12px; color:gray;">${log.new_value || '-'}</td></tr>`;
            }).join('');
        } catch (e) {}
    };

    window.openCoWorkerModal = async function(id) {
        document.getElementById('coworker-modal').style.display = 'flex'; document.getElementById('cw-edit-id').value = id; const listDiv = document.getElementById('cw-edit-list'); listDiv.innerHTML = '⏳ กำลังโหลด...';
        try {
            const { data: c } = await supabaseClient.from('clearances').select('co_worker_ids, member_id').eq('id', id).single(); const existingIds = c.co_worker_ids || [];
    ...
    window.saveCoWorkersQuick = async function() {
        const id = document.getElementById('cw-edit-id').value; const coWorkerIds = []; document.querySelectorAll('.cw-quick-cb:checked').forEach(cb => coWorkerIds.push(cb.value));
        const btn = document.querySelector('#coworker-modal .btn-primary'); btn.disabled = true; btn.textContent = 'กำลังบันทึก...';
        try { await supabaseClient.from('clearances').update({ co_worker_ids: coWorkerIds }).eq('id', id); alert('✅ อัปเดตรายชื่อเพื่อนสำเร็จ!'); document.getElementById('coworker-modal').style.display = 'none'; window.loadClearanceHistory(); window.loadPendingRequests(); } 
    ...
    window.loadSettingsData = async function() {
        const { data: banks } = await supabaseClient.from('bank_accounts').select('*'); const bTbody = document.querySelector('#manage-bank-table tbody'); if(bTbody) bTbody.innerHTML = banks.map(b => `<tr><td>${b.bank_name}</td><td>฿${b.balance.toLocaleString()}</td><td style="text-align:center;"><button onclick="deleteBank('${b.id}')" style="background:none; border:none; color:red; cursor:pointer;">🗑️</button></td></tr>`).join('');
        const { data: funds } = await supabaseClient.from('funds').select('*'); const fTbody = document.querySelector('#manage-fund-table tbody'); if(fTbody) fTbody.innerHTML = funds.map(f => `<tr><td>${f.fund_name}</td><td>฿${f.remaining_budget.toLocaleString()}</td><td style="text-align:center;"><button onclick="deleteFund('${f.id}')" style="background:none; border:none; color:red; cursor:pointer;">🗑️</button></td></tr>`).join('');
    };

    window.addBank = async function() { const name = document.getElementById('add-bank-name').value; const bal = parseFloat(document.getElementById('add-bank-bal').value); if(!name || isNaN(bal)) return alert("กรุณากรอกข้อมูลให้ครบ"); const subBtn = event.target; subBtn.disabled = true; subBtn.textContent = "กำลังเพิ่ม..."; try { await supabaseClient.from('bank_accounts').insert([{ bank_name: name, balance: bal }]); document.getElementById('add-bank-name').value = ''; document.getElementById('add-bank-bal').value = ''; alert("✅ เพิ่มบัญชีสำเร็จ!"); window.loadAllAdminData(); } catch (err) { alert("❌ ไม่สามารถเพิ่มบัญชีได้: " + err.message); } finally { subBtn.disabled = false; subBtn.textContent = "+ เพิ่มบัญชี"; } };
    ...
    window.addFund = async function() { const name = document.getElementById('add-fund-name').value; const bal = parseFloat(document.getElementById('add-fund-bal').value); if(!name || isNaN(bal)) return alert("กรุณากรอกข้อมูลให้ครบ"); const subBtn = event.target; subBtn.disabled = true; subBtn.textContent = "กำลังเพิ่ม..."; try { await supabaseClient.from('funds').insert([{ fund_name: name, remaining_budget: bal }]); document.getElementById('add-fund-name').value = ''; document.getElementById('add-fund-bal').value = ''; alert("✅ เพิ่มกองทุนสำเร็จ!"); window.loadAllAdminData(); } catch (err) { alert("❌ ไม่สามารถเพิ่มกองทุนได้: " + err.message); } finally { subBtn.disabled = false; subBtn.textContent = "+ เพิ่มกองทุน"; } };
    ...
    window.viewTransaction = async function(id) {
        document.getElementById('view-tx-modal').style.display = 'flex'; const content = document.getElementById('view-tx-content'); content.innerHTML = 'กำลังโหลดข้อมูล...';
        try {
            const { data: tx } = await supabaseClient.from('transactions').select(`*`).eq('id', id).single(); if (!tx) throw new Error("ไม่พบข้อมูล"); const pMap = await getProfileMap();
            const { data: bData } = await supabaseClient.from('bank_accounts').select('bank_name').eq('id', tx.bank_account_id).single(); const { data: fData } = await supabaseClient.from('funds').select('fund_name').eq('id', tx.fund_id).single();
            ...
            content.innerHTML = `<table style="width:100%; font-size:14px;"><tr><td style="padding:5px 0; color:gray; width:30%;">วันที่:</td><td style="font-weight:bold;">${date}</td></tr><tr><td style="padding:5px 0; color:gray;">รายละเอียด:</td><td>${tx.description || '-'}</td></tr><tr><td style="padding:5px 0; color:gray;">ฝ่าย / แผนก:</td><td style="color:var(--primary);">${tx.department || 'ส่วนกลาง'}</td></tr><tr><td style="padding:5px 0; color:gray;">สถานที่:</td><td>${tx.location || '-'}</td></tr><tr><td style="padding:5px 0; color:gray;">ยอดเงิน:</td><td style="font-weight:bold; color:var(--primary);">฿${parseFloat(tx.amount).toLocaleString()}</td></tr><tr><td style="padding:5px 0; color:gray;">บัญชี / กองทุน:</td><td>🏦 ${bData?.bank_name||'-'} <br> 💼 ${fData?.fund_name||'-'}</td></tr><tr><td style="padding:5px 0; color:gray;">ผู้บันทึก:</td><td>${pMap[tx.created_by]?.full_name || 'แอดมิน'}</td></tr></table>${imgHtml}<div style="margin-top:20px; padding-top:15px; border-top:1px dashed #ccc; display:flex; justify-content:center;"><button onclick="undoTransaction('${tx.id}')" class="btn btn-danger" style="width:100%; padding:10px; font-size:14px;">🗑️ ยกเลิกรายการนี้ (Undo & คืนเงิน)</button></div>`;
        } catch (err) { content.innerHTML = '<span style="color:red;">โหลดข้อมูลไม่สำเร็จ</span>'; }
    };

    window.viewClearance = async function(id) {
        document.getElementById('view-clearance-modal').style.display = 'flex'; const content = document.getElementById('view-clearance-content'); content.innerHTML = 'กำลังโหลดข้อมูล...';
        try {
            const { data: c } = await supabaseClient.from('clearances').select(`*`).eq('id', id).single(); const { data: items } = await supabaseClient.from('clearance_items').select('*').eq('clearance_id', id); const pMap = await getProfileMap();
            ...
            content.innerHTML = `<div style="display:flex; gap:20px; flex-wrap:wrap;"><div style="flex:1; min-width:300px;"><table style="width:100%; font-size:14px;"><tr><td style="padding:4px 0; color:gray; width:35%;">ผู้เบิก:</td><td style="font-weight:bold;">${pMap[c.member_id]?.full_name||'-'}</td></tr><tr><td style="padding:4px 0; color:gray;">ฝ่าย / แผนก:</td><td style="color:var(--primary);">${c.department || '-'}</td></tr><tr><td style="padding:4px 0; color:gray;">หัวข้อ:</td><td>${c.purpose}</td></tr><tr><td style="padding:4px 0; color:gray;">ยอดขอเบิกล่วงหน้า:</td><td>฿${parseFloat(c.requested_amount).toLocaleString()}</td></tr><tr><td style="padding:4px 0; color:gray;">ยอดใช้จ่ายจริง:</td><td style="font-weight:bold; color:var(--success);">฿${parseFloat(c.total_actual_amount).toLocaleString()}</td></tr><tr><td style="padding:4px 0; color:gray;">บัญชีรับเงิน (Member):</td><td>${c.member_bank_details || '-'}</td></tr></table>${imgsHtml}</div><div style="flex:1.2; min-width:300px;"><h4 style="margin:0 0 5px 0; color:var(--primary);">🛒 รายการสินค้า / บิลย่อย</h4>${itemsHtml}</div></div>`;
        } catch (err) { content.innerHTML = '<span style="color:red;">โหลดข้อมูลไม่สำเร็จ</span>'; }
    };

    // 🌟 แก้ไข: ดัก Error Export CSV และป้องกันบั๊กในมือถือ
    window.exportLedgerToCSV = async function() {
        ...
        try {
            ...
            const pMap = await getProfileMap(); const { data: bData } = await supabaseClient.from('bank_accounts').select('*'); const bankMap = {}; bData?.forEach(b => bankMap[b.id] = b.bank_name); const { data: fData } = await supabaseClient.from('funds').select('*'); const fundMap = {}; fData?.forEach(f => fundMap[f.id] = f.fund_name);
            ...
            txs.forEach(tx => {
                ...
                desc = desc.replace(/"/g, '""'); // 🌟 กันพังเวลาพิมพ์เครื่องหมายคำพูดในชื่อ
                ...
                if (match) {
                    ...
                    if (items.length > 0) { items.forEach(it => { 
    ...
    window.approveUser = async function(id, currentName) {
        ...
        try { await supabaseClient.from('profiles').update({ status: 'approved', role: isSetAsAdmin ? 'admin' : 'member', full_name: finalName }).eq('id', id); alert(`🎉 อนุมัติคุณ ${finalName} เรียบร้อยแล้ว!`); window.loadPendingUsers(); } catch (err) { alert("❌ เกิดข้อผิดพลาด: " + err.message); }
    };
    ...
    window.undoTransaction = async function(txId) {
        ...
        try {
            const { data: tx } = await supabaseClient.from('transactions').select('*').eq('id', txId).single(); if (!tx) throw new Error("ไม่พบข้อมูลรายการนี้");
            ...
            const { data: bData } = await supabaseClient.from('bank_accounts').select('balance').eq('id', tx.bank_account_id).single(); const { data: fData } = await supabaseClient.from('funds').select('remaining_budget').eq('id', tx.fund_id).single();
            ...
            await supabaseClient.from('bank_accounts').update({ balance: nBal }).eq('id', tx.bank_account_id); await supabaseClient.from('funds').update({ remaining_budget: nFun }).eq('id', tx.fund_id);

            if (tx.clearance_id) { let revertStatus = tx.description.includes('[โอนตั้งต้น]') ? 'pending_advance' : 'pending_clearance'; await supabaseClient.from('clearances').update({ status: revertStatus }).eq('id', tx.clearance_id); await supabaseClient.from('transactions').delete().eq('id', txId); } 
            else { if (tx.transaction_type === 'income' || tx.transaction_type === 'expense') await supabaseClient.from('transactions').delete().eq('id', txId); else await supabaseClient.from('transactions').update({ status: 'pending' }).eq('id', txId); }
    ...
    window.loadAllAdminData = async function() {
        ...
        try {
            const pMap = await getProfileMap(); if (pMap[currentUser.id] && document.getElementById('current-user-name')) document.getElementById('current-user-name').textContent = pMap[currentUser.id].full_name || 'Admin';
            const { count: c1 } = await supabaseClient.from('clearances').select('*', { count: 'exact', head: true }).in('status', ['pending_advance', 'pending_clearance']); const { count: c2 } = await supabaseClient.from('transactions').select('*', { count: 'exact', head: true }).eq('status', 'pending'); const totalPending = (c1 || 0) + (c2 || 0);
            const badge = document.getElementById('noti-badge');
            if (badge) { if (totalPending > 0) { badge.textContent = totalPending; badge.style.display = 'inline-block'; document.getElementById('noti-bell').onclick = () => alert(`🚨 มีรายการรอตรวจสอบทั้งหมด ${totalPending} รายการ`); } else { badge.style.display = 'none'; document.getElementById('noti-bell').onclick = () => alert(`✅ ไม่มีรายการค้างตรวจสอบ`); } }
        } catch(e) { console.error(e); }
        window.loadDashboardWidgets(); window.loadPendingDonations(); window.loadPendingRequests(); window.loadLedger(); window.loadClearanceHistory(); window.loadSettingsData(); window.loadPendingUsers(); if (typeof window.loadAuditLogs === 'function') window.loadAuditLogs();
    };
